...
export async function generateMetadata({ params: { slugId } }: { params: { slugId: string } }) {
  ...
  return {
    title: `The Wander Shop - ${productData?.name}`,
    description: `${productData.description}`,
    keywords: `The Wander Shop - ${productData?.name} - ${productData.slug}`,
    openGraph: {
  ...
  return (
    ...
    >
      <Head>
        ...
        <meta name='author' content='The Wander Shop' />
        <meta name='name' content='Khóa học NextJS 14 typescript PRO 2024' />
        <meta name='image' content={productData?.image} />
        {/* facebook */}
        <meta property='og:type' content='website' />
        <meta property='og:title' content={`The Wander Shop - ${productData?.name}`} />
        <meta property='og:description' content={description} />
        <meta property='og:image' content={productData?.image} />
        {/* twitter */}
        <meta property='twitter:card' content='website' />
        <meta property='twitter:title' content={`The Wander Shop - ${productData?.name}`} />
        <meta property='twitter:description' content={productData?.description} />
        <meta property='twitter:image' content={`The Wander Shop - ${productData?.name}`} />
      </Head>
